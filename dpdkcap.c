...
int main(int argc, char *argv[]) {
  ...
  struct core_redis_config   * cores_config_redis_list;
  ...
  int result;
  FILE * log_file;
  struct info *table_list[HASH_TABLE_SIZE*4];
  ...
  cores_config_capture_list=
    malloc(sizeof(struct core_capture_config)*arguments.per_port_c_cores
        *nb_ports);
  cores_config_redis_list=
    malloc(sizeof(struct core_redis_config)*arguments.num_w_cores);
  ...
  /* Init hash table */
    struct info **table = &(table_list[i*65535]);	
    initTable(table); 

    //Configure count core
    ...
    config->table = cores_config_redis_list[i].table;
  ...
  /* Configure redis core */
  for(i=0; i<arguments.num_w_cores;i++){
    struct core_redis_config * config = &(cores_config_redis_list[i]);
    initHashTable(config->table);
    /* Launch update core */
    if(rte_eal_remote_launch((lcore_function_t *)updateStatus,config,core_index)<0)
	rte_exit(EXIT_FAILURE, "Could not launch update process on lcore "\
	"%d.\n",core_index);
    core_index = rte_get_next_lcore(core_index, SKIP_MASTER, 0);
  }
